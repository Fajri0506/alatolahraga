...
const PengembalianModel = {
    ...
    try {
      ...
      const [result] = await connection.query(
        `INSERT INTO pengembalian (id_penyewaan, tanggal_dikembalikan, kondisi_kembali, denda, catatan) 
         VALUES ($1, $2, $3, $4, $5) RETURNING id_pengembalian`,
        [id_penyewaan, tanggal_dikembalikan, kondisi_kembali || 'baik', denda || 0, catatan || null]
      );
      const id_pengembalian = result[0]?.id_pengembalian;

      // 2. Update penyewaan status to 'selesai'
      await connection.query(
        "UPDATE penyewaan SET status = 'selesai' WHERE id_penyewaan = $1",
        [id_penyewaan]
      );
      ...
      if (kondisi_kembali === 'baik') {
        const [details] = await connection.query(
          'SELECT id_alat, jumlah FROM detail_penyewaan WHERE id_penyewaan = $1',
          [id_penyewaan]
        );
        ...
        const [details] = await connection.query(
          'SELECT id_alat FROM detail_penyewaan WHERE id_penyewaan = $1',
          [id_penyewaan]
        );
        for (const item of details) {
          await connection.query(
            'UPDATE alat_olahraga SET kondisi = $1 WHERE id_alat = $2',
  ...
  getByRentalId: async (rentalId) => {
    const [rows] = await db.query('SELECT * FROM pengembalian WHERE id_penyewaan = $1', [rentalId]);
    return rows[0];
  }
};
